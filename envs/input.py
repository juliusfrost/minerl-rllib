...
class MineRLInputReader(InputReader):
    def __init__(self, ioctx: IOContext = None):
        ...
        self.data = minerl.data.make(env_name,
                                     data_dir=os.getenv('MINERL_DATA_ROOT', input_config.get('data_dir', 'data')),
        ...
        self.generator = self.data.batch_iter(batch_size, seq_len, num_epochs=num_epochs,
                                              preload_buffer_size=preload_buffer_size, seed=seed,
    ...
    def process_batch(self, batch):
        ...
        action = [[self.process_reverse_action({'vector': action['vector'][i][t]}) for i in range(batch_n)] for t in
                  range(batch_t)]
        reward = [[self.process_reward(reward[i][t]) for i in range(batch_n)] for t in range(batch_t)]
        obs = [[self.process_obs({'pov': obs['pov'][i][t], 'vector': obs['vector'][i][t]}) for i in range(batch_n)] for
               t in range(batch_t)]
        next_obs = [[self.process_obs({'pov': next_obs['pov'][i][t], 'vector': next_obs['vector'][i][t]}) for i in
                     range(batch_n)] for t in range(batch_t)]
        # print(len(obs), len(obs[0]), obs[0][0].shape)
        return np.array(obs).squeeze(1), np.array(action).squeeze(1), np.array(reward).squeeze(1), np.array(
            next_obs).squeeze(1), np.transpose(done).squeeze(1)
