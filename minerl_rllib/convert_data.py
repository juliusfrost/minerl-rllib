...
parser.add_argument(
    "--env", type=str, default=None, help="Environment name to write jsons"
)
parser.add_argument(
    "-f",
    "--config-file",
    default=None,
    type=str,
    help="config file to load environment config",
)
parser.add_argument(
    "--env-config",
    default="{}",
...
parser.add_argument(
    "--overwrite",
    action="store_true",
    help="overwrite existing data if directory not empty",
)
...
def main():
    ...
    for env_name in env_list:
        ...
        write_jsons(
            env_name, args.data_dir, env_config, env_save_path, overwrite=args.overwrite
        )
