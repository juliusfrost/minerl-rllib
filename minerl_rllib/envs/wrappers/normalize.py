...
class MineRLNormalizeObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env, pov_low=0., pov_high=1., vec_low=-1., vec_high=1.):
        super().__init__(env)
        ...
        self._pov_space = gym.spaces.Box(pov_low, pov_high, self._old_pov_space.low.shape, np.float32)
        self._vec_space = gym.spaces.Box(vec_low, vec_high, self._old_vec_space.low.shape, np.float32)
        self.observation_space = gym.spaces.Tuple((self._pov_space, self._vec_space))

    def observation(self, observation):
        pov, vec = observation
        pov = normalize(pov, self._old_pov_space.low, self._old_pov_space.high, self._pov_space.low,
                        self._pov_space.high)
        vec = normalize(vec, self._old_vec_space.low, self._old_vec_space.high, self._vec_space.low,
...
class MineRLNormalizeActionWrapper(gym.ActionWrapper):
    def __init__(self, env, low=-1., high=1.):
        ...
        self._vec_space = gym.spaces.Box(low, high, self._old_vec_space.low.shape, np.float32)
        self.action_space = self._vec_space

    def action(self, action):
        return normalize(action, self._old_vec_space.low, self._old_vec_space.high, self._vec_space.low,
                         self._vec_space.high)

    def reverse_action(self, action):
        action = self.env.reverse_action(action)
        return normalize(action, self._vec_space.low, self._vec_space.high, self._old_vec_space.low,
