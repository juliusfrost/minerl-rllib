    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        if self.use_prev_action_reward:
            ...
            state_inputs = torch.cat(
                (pov_embed, vector_embed, action_embed, reward_embed), dim=-1
            )
        else:
        ...
        if self.use_rnn:
            ...
            self._logits = torch.reshape(
                rnn_output, (batch_t * batch_n, self._rnn.hidden_size)
            )
        else:
